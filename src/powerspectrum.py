...
def detect_host_with_powerspectrum(sci_image=None, tpl_image=None, number_of_iterations=1000, cutout_sizes=[7, 15, 29],
    ...
    output_table = at.Table(names=['IMAGE_TYPE', 'CUTOUT_SIZE', 'STATISTIC', 'PVALUE'],
    ...
    if np.isin(metric, ['anderson-darling', 'kstest'], invert=True):
        raise Exception(
            "Input metric has not been integrated into the pipeline yet. Please choose either 'anderson-darling' or 'kstest'.")
    ...
    for i, image in enumerate([sci_image, tpl_image]):
        ...
        for size in cutout_sizes:
            ...
            statistic_name = metric + "_" + image_type_dict[i] + "_" + str(size) + "_statistic"
            pvalue_name = metric + "_" + image_type_dict[i] + "_" + str(size) + "_pvalue"
